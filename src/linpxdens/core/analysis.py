"""
Core analysis module for linear pixel density and structure evaluation.

This module provides the main computational pipeline for analyzing images
based on regions of interest (ROIs). It supports:

- Loading and validating image inputs
- Fitting geometric lines within ROIs
- Collecting multiple fitted lines
- Computing statistical spacing between detected structures

Main workflow:
    1. Load image from file path
    2. Define ROIs (regions of interest)
    3. Fit lines inside each ROI
    4. Collect results in a LineCollection
    5. Compute mean distance and statistics between lines

Key functions:
- load_image: Load and validate image from disk
- fit_line: Fit a line within a given ROI
- analyze: High-level pipeline combining all steps
- get_mean_distance: Compute spacing statistics from line collection
"""
...
def analyze(image_path, rois):
    ...
    :param image_path: Path to the image file.
    :param rois: List or array of Tuples (x, y, w, h) defining the ROIs.
